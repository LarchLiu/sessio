import { addDynamicIconSelectors } from "@iconify/tailwind";

/** @type {import('tailwindcss').Config} */
export default {
  content: ["./index.html", "./src/**/*.{ts,tsx}"],
  theme: {
    extend: {
      fontFamily: {
        sans: [
          "-apple-system",
          "BlinkMacSystemFont",
          "SF Pro Text",
          "Segoe UI",
          "Roboto",
          "Helvetica Neue",
          "sans-serif",
        ],
        mono: ["SF Mono", "Menlo", "Monaco", "Consolas", "monospace"],
      },
      fontSize: {
        caption: ["12px", { lineHeight: "1.4", letterSpacing: "0" }],
        meta: ["12px", { lineHeight: "1.45", letterSpacing: "0" }],
        "body-sm": ["13px", { lineHeight: "1.5" }],
        body: ["14px", { lineHeight: "1.5" }],
        subtitle: ["16px", { lineHeight: "1.4" }],
        title: ["17px", { lineHeight: "1.3" }],
        hero: ["20px", { lineHeight: "1.2" }],
      },
      colors: {
        ink: "rgb(var(--color-fg) / <alpha-value>)",
        surface: {
          DEFAULT: "rgb(var(--color-bg) / <alpha-value>)",
          sidebar: "rgb(var(--color-bg-sidebar) / <alpha-value>)",
          panel: "rgb(var(--color-bg-panel) / <alpha-value>)",
          "panel-alt": "rgb(var(--color-bg-panel-alt) / <alpha-value>)",
        },
        card: {
          DEFAULT: "rgb(var(--color-card-bg) / <alpha-value>)",
          panel: "rgb(var(--color-card-panel-bg) / <alpha-value>)",
          border: "rgb(var(--color-card-border) / <alpha-value>)",
          hover: "rgb(var(--color-card-hover) / <alpha-value>)",
          "action-hover": "rgb(var(--color-card-action-hover) / <alpha-value>)",
          active: "rgb(var(--color-card-active) / <alpha-value>)",
          fg: "rgb(var(--color-card-fg) / <alpha-value>)",
          muted: "rgb(var(--color-card-muted) / <alpha-value>)",
          subtle: "rgb(var(--color-card-subtle) / <alpha-value>)",
          icon: "rgb(var(--color-card-icon) / <alpha-value>)",
          chip: {
            DEFAULT: "rgb(var(--color-card-chip-bg) / <alpha-value>)",
            fg: "rgb(var(--color-card-chip-fg) / <alpha-value>)",
          },
        },
        input: {
          DEFAULT: "rgb(var(--color-input-bg) / <alpha-value>)",
          border: "rgb(var(--color-input-border) / <alpha-value>)",
          fg: "rgb(var(--color-input-fg) / <alpha-value>)",
          placeholder: "rgb(var(--color-input-placeholder) / <alpha-value>)",
          focus: "rgb(var(--color-input-focus-border) / <alpha-value>)",
        },
        tooltip: {
          bg: "rgb(var(--color-tooltip-bg) / <alpha-value>)",
          fg: "rgb(var(--color-tooltip-fg) / <alpha-value>)",
        },
        brand: "rgb(var(--color-brand) / <alpha-value>)",
        blue: "rgb(var(--color-blue) / <alpha-value>)",
        "accent-purple": "rgb(var(--color-accent-purple) / <alpha-value>)",
        emerald: "rgb(var(--color-emerald) / <alpha-value>)",
        status: {
          warn: "rgb(var(--color-status-warn) / <alpha-value>)",
          error: "rgb(var(--color-status-error) / <alpha-value>)",
        },
      },
    },
  },
  plugins: [addDynamicIconSelectors()],
};
