...
const CLICK_ARG = {
  ...
  properties: {
    ...
    dispatchRoute: { type: "string", enum: ["auto", "ax", "target_pid", "hid"] },
    dispatch_route: { type: "string", enum: ["auto", "ax", "target_pid", "hid"] },
  },
  required: ["snapshotId"],
...
const POINT_ARG = {
  ...
  properties: {
    ...
    coordSpace: { type: "string", enum: ["screenshot", "screen"] },
    coord_space: { type: "string", enum: ["screenshot", "screen"] },
    dispatchRoute: { type: "string", enum: ["auto", "target_pid", "hid"] },
    dispatch_route: { type: "string", enum: ["auto", "target_pid", "hid"] },
  },
...
const TOOL_PARAMETERS: Record<ToolName, unknown> = {
  ...
  computer_click_element: {
    ...
    anyOf: [{ required: ["elementId"] }, { required: ["ref"] }],
    additionalProperties: false,
  },
...
const TOOL_DESCRIPTIONS: Record<ToolName, string> = {
  ...
  computer_click: "Primary click with optional dispatchRoute. Default auto uses AX->target_pid->hid for element refs and target_pid->hid for coordinates. Returns a post-action AppState plus lastClickResult { route, outcome }.",
  computer_click_element: "Click an accessibility element from the latest snapshot. dispatchRoute supports auto|ax|target_pid|hid. Returns a post-action AppState plus lastClickResult { route, outcome }.",
  computer_click_at: "Click a screenshot coordinate from the latest snapshot. dispatchRoute supports auto|target_pid|hid. Returns a post-action AppState plus lastClickResult { route, outcome }.",
  computer_secondary_click: "Secondary-click an AX element or coordinate from the latest snapshot. dispatchRoute supports auto|ax|target_pid|hid. Returns a post-action AppState plus lastActionResult { kind, route, outcome }.",
  computer_perform_secondary_action:
    "Skill-compatible alias for secondary-click. dispatchRoute supports auto|ax|target_pid|hid. Returns a post-action AppState plus lastActionResult { kind, route, outcome }.",
  computer_double_click: "Double-click a screenshot coordinate from the latest snapshot. dispatchRoute supports auto|target_pid|hid. Returns a post-action AppState plus lastActionResult { kind, route, outcome }.",
  computer_drag: "Drag between two screenshot coordinates from the latest snapshot. dispatchRoute supports auto|target_pid|hid. Returns a post-action AppState plus lastActionResult { kind, route, outcome }.",
  computer_set_value: "Set an accessibility element value directly. Returns a post-action AppState plus lastActionResult { kind, route, outcome }.",
  ...
  computer_scroll: "Scroll the target or an AX element from the latest snapshot. dispatchRoute supports auto|ax|target_pid|hid. Returns a post-action AppState plus lastActionResult { kind, route, outcome }.",
  ...
  for (const name of TOOL_NAMES) {
    pi.registerTool({
      ...
      promptGuidelines: [
        ...
        "For primary clicks, inspect the returned screenshot first and treat lastClickResult as a hint: semantic_success/observed_effect are strong positives, uncertain means stop and inspect, and no_effect is the only immediate-retry signal. If outcome=no_effect, retry only after re-reading the screenshot, and when retrying prefer the next explicit dispatchRoute in order instead of repeating auto blindly.",
        "Secondary click, double click, drag, and scroll also accept dispatchRoute. Leave it as auto unless you are intentionally forcing the next lower-level route after inspecting the latest screenshot.",
