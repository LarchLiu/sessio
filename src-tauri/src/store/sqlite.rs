...
use crate::models::{
    ...
    AssistantInfo, AstraConfig, CanvasBlockRecord, CanvasContextAnchor, CanvasDocumentInfo,
...
#[cfg(test)]
use crate::models::{
    AssistantType, PlanRoundMode, PlanRoundStatus, PlanTaskStatus, ProjectStageType,
    SessionHistoryTurn, StageType,
};
#[cfg(test)]
...
pub(super) fn usage_list(items: &[String]) -> String {
    const LIMIT: usize = 8;
...
impl SessionStore for SqliteStore {
    ...
    fn list_project_stages(&self, project_id: &str) -> Result<Vec<ProjectStageInfo>> {
        let conn = self.conn.lock().unwrap();
        stages::list_project_stages(&conn, project_id)
    }
    ...
    fn list_process_template_stages(
        ...
        let conn = self.conn.lock().unwrap();
        stages::list_process_template_stages(&conn, process_template_id)
    }
    ...
    fn update_project_stage(
        ...
        let mut conn = self.conn.lock().unwrap();
        stages::update_project_stage(&mut conn, stage_id, patch)
    }
    ...
    fn update_project_stage_assistants(
        ...
        let mut conn = self.conn.lock().unwrap();
        stages::update_project_stage_assistants(&mut conn, stage_id, assistant_ids)
    }

    fn delete_project_stage(&self, stage_id: &str) -> Result<()> {
        let conn = self.conn.lock().unwrap();
        stages::delete_project_stage(&conn, stage_id)
    }
