...
mod migration_tests {
    ...
    #[test]
    fn thread_replay_maps_astra_planner_sessions_to_rounds() {
        let path = unique_db("sessio-thread-replay-planner-rounds");
        let store = SqliteStore::open(&path).unwrap();
        store.init().unwrap();
        let parent = temp_child_path(
            &std::env::temp_dir(),
            "sessio-thread-replay-planner-rounds-parent",
        );
        std::fs::create_dir(&parent).unwrap();
        ...
        let project = store
            .add_project(
                &parent.to_string_lossy(),
                Some("Planner Replay Project"),
                "code".to_string(),
                None,
            )
            .unwrap();
        let thread = store
            .create_thread(&project.id, "Map planner sessions to rounds", None)
            .unwrap();
        for (id, updated_at) in [("planner-session-1", 20), ("planner-session-2", 40)] {
            let session = SessionInfo {
                id: id.to_string(),
                agent: Agent::AstraPi,
                forked_from_agent: None,
        ...
        let round_one = store
            .create_plan_round(NewPlanRound {
                thread_id: &thread.id,
                astra_run_id: Some("replay-run"),
                round_index: Some(0),
                summary: Some("First round"),
                mode: PlanRoundMode::Parallel,
                source: PlanRoundSource::Astra,
                status: PlanRoundStatus::Completed,
                tasks: vec![NewPlanTask {
                    thread_stage_id: None,
                    assistant_id: None,
                    agent_participant_id: None,
            ...
            .upsert_astra_run(&AstraRunRecord {
                ...
                internal_planner_sessions: vec![
                    ...
                    AstraRunSessionRecord {
                        run_id: "replay-run".to_string(),
                        agent: Agent::AstraPi,
                        session_id: "planner-session-2".to_string(),
                        role: PlanTaskSessionRole::Planner,
                        sort_order: 1,
                        created_at: 30,
                        updated_at: 40,
        ...
        let replay = store.get_thread_replay(&thread.id).unwrap();
        let planner_round = |session_id: &str| {
            replay
                .sessions
                .iter()
                ...
                .and_then(|session| {
                    session
                        .sources
                        .iter()
                        .find(|source| source.kind == ThreadReplaySessionSourceKind::AstraInternal)
                })
                .and_then(|source| source.plan_round_id.as_deref())
                .map(str::to_string)
        };

        assert_eq!(planner_round("planner-session-1"), Some(round_one.id));
        assert_eq!(planner_round("planner-session-2"), Some(round_two.id));
