...
fn load_sessions_from_store_or_scan(db_path: Option<&str>) -> Result<Vec<crate::models::SessionInfo>> {
    match open_store(db_path) {
        ...
        Err(e) => {
            eprintln!("sessio: failed to open session index ({e}), falling back to filesystem scan.");
            Ok(providers::list_all())
        }
    }
}
