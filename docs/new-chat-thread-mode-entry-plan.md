...
在 New Chat 输入框底部第二个 selector 位置，将原 Kanban item 改为 thread kind 入口，支持 `process`、`teamwork`、`brainstorm`、`debate` 四种模式。用户选择模式后，New Chat 从普通会话入口变为 thread 创建入口：根据模式展示不同配置项，发送时创建 thread，并进入已新增的 `ThreadMultiSessionChatPage`。四种 thread mode 都会启动对应 Astra run；`process` 使用 deterministic backend 按用户定义的 stage 顺序执行。
...
- 按发送后先创建 thread；`process` 添加所选 stages 后调用 `createAstraRun(thread.id, null)`，由 deterministic backend 按 stage/assistant 顺序执行；`teamwork` / `brainstorm` / `debate` 也调用 `createAstraRun(thread.id, null)` 并进入各自的编排 backend。
...
- mode 为 `process`、`teamwork`、`brainstorm`、`debate` 任意一种：创建 thread 后设置 `selectedThread` + `detailMode = "threadMultiSessionChat"`，最终进入 `ThreadMultiSessionChatPage`。四种 mode 都在跳转前启动对应 Astra run。
...
- 发送按钮按 mode 切换视觉语义：
  ...
  - `Process` / `Teamwork` / `Brainstorm` / `Debate`：把 primary send button 替换为 `ThreadMultiSessionChatPage` 里 Astra start button 的 `Sparkles` shimmer 样式，表示创建 thread、启动对应 Astra run 并进入 multi-session 页面。
  - thread mode 发送中使用同位置 `LoaderCircle`，不要额外显示第二个 Astra 按钮。
  - thread mode tooltip/aria label 使用 `Create thread` 或更具体的 `Create process thread` / `Create debate thread`。
- Kanban item selector 从 New Chat 移除；Kanban 后续如仍需关联，可放到 Project/Workbench 或 session detail 的次级操作里，不作为 New Chat 主流程。

## 模式配置

- `process`
  - 弹出当前 project 可用的 process-template/project stages。
  - 支持选择和排序 stages。
  - 创建 thread 后通过现有 `addThreadStage` 添加选中的 stages。
  - 添加 stages 后立即启动 deterministic process run；run 按 stage 顺序和 stage assistant 顺序执行，空 assistant stage 作为人工 checkpoint。
...
- 修改 `src/pages/NewChatPage.tsx`
  ...
  - 四种 thread mode 创建 thread 后调用 `createAstraRun`，不要再调用 `composer.runStartSession` 创建首条 linked session。
  - `process` 创建 thread 和 stages 后立即启动 deterministic process run，再进入 `ThreadMultiSessionChatPage`。
...
- 扩展 thread API
  ...
  - `process`、`teamwork` 继续保留现有 `assistantIds` 行为。
...
- 前端
  ...
  - `process` 模式能选择、排序 stages，并创建带 stages 的 thread。
...
- `process` 和 `teamwork` 不在本轮改成 agent-based。
